...
class SoundDetection(object):
    ...
    def sound_detection(self):
        return self.__get_config("AudioMutation")

    def is_sound_detector_on(self):
    ...
    def is_record_on_sound_detection(self):
        ret = self.motion_detection
        status = [s for s in ret.split() if '.RecordEnable=' in s][0]\
            .split('=')[-1]
        return str2bool(status)  # pylint: disable=no-value-for-parameter

    @sound_detection.setter
    def sound_detection(self, opt):
